...
(async () => {
  core.invoke = async (cmd: string, args: any) => {
    if (cmd === 'load_settings') return { output: '/tmp/out.mp4', uiFont: 'Arial', theme: 'dark', language: 'fr', watermarkOpacity: 0.8, watermarkScale: 0.3, defaultWidth: 1920, defaultHeight: 1080, defaultFps: 30, defaultPrivacy: 'private', defaultPlaylistId: 'PL123' };
    if (cmd === 'save_settings') {
      ...
      assert.strictEqual(args.settings.defaultFps, 30);
  ...
  assert.strictEqual(s.defaultFps, 30);
  assert.strictEqual(s.defaultPrivacy, 'private');
  assert.strictEqual(s.defaultPlaylistId, 'PL123');
  await saveSettings({ output: '/tmp/out.mp4', uiFont: 'Arial', theme: 'dark', language: 'fr', watermarkOpacity: 0.8, watermarkScale: 0.3, defaultWidth: 1920, defaultHeight: 1080, defaultFps: 30, defaultPrivacy: 'private', defaultPlaylistId: 'PL123' });
